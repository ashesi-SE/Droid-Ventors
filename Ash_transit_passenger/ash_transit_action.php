<?php
	...
	function get_trip_details(){
		...
		echo "{";
			...
			echo jsons("numPeople_Reserved",$row['numPeopleReserved']).",";
			echo jsons("current_location",$row['cur_location']);
			echo "}";
			echo "}";
	}
    ...
    . "From=AshTransit"
    . "&To={$pno}"
    . "&Content=Dear+passenger+Your+reservation+for+trip+{$tname}+on+{$md}+was+successful"
    . "&ClientId=odfbifrp"
	...
	function update_trip(){
		...
		if(!$obj->update_trip($s,$n)){
		echo "{";
			echo '"booking":{';
			//echo jsonn("result",0). ",";
			echo jsons("message","Trip cannot be updated");
			echo mysql_error();
			echo "}";
			echo "}";
			return;
			...
			echo jsonn("result",1) .",";
			echo '"book":{';
			echo jsons("message","Trip successfully updated");
			echo "}";
			echo "}";
		}
